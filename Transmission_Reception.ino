...
/*! \brief 
 * - La déclaration unsigned char data crée une variable data capable de stocker des entiers non signés de 8 bits. 
 * - Elle est utilisée pour stocker des données reçues via la communication série USART. 
 * - La taille de la variable est limitée à des valeurs entre 0 et 255.
 */
unsigned char data;

/*! \brief 
 * - La déclaration unsigned char flag = 0; crée une variable nommée flag de type entier non signé de 8 bits, initialement mise à 0. 
 * - Cette variable est souvent utilisée comme un indicateur (drapeau) dans les programmes pour signaler un état particulier. 
 * - Dans ce contexte, elle est employée pour indiquer si des données ont été reçues via la communication série USART, avec la valeur 1 pour indiquer la réception de données et 0 sinon.
 */
unsigned char flag =0;

/*! \brief 
 * - La fonction USART_Init initialise la communication série USART sur un microcontrôleur AVR. 
 * - Elle configure le registre de baud rate avec la valeur fournie, active le récepteur et le transmetteur USART, active l'interruption de réception pour permettre l'utilisation d'interruptions lors de la réception de données, et fixe le format du cadre de communication avec 8 bits de données et 1 bit d'arrêt. 
 * - Cette configuration standard assure une communication asynchrone avec des caractéristiques spécifiques. 
 * - L'utilisation de la fonction USART_Init simplifie la configuration de la communication série dans les programmes principaux en appelant simplement cette fonction.
 */
...
/*! \brief 
 * - La fonction USART_Receive attend que le tampon de réception soit prêt en vérifiant le bit d'indicateur d'attente de tampon plein (RXC0), puis retourne les données reçues du registre de données de réception (UDR0). 
 * - Cette fonction facilite la récupération synchrone des données reçues sur la communication série USART sans risque de lire des données non prêtes.
 */
...
/*! \brief 
 * - Cette interruption de réception USART (ISR(USART_RX_vect)) est une fonction à exécuter lorsque des données sont reçues sur la communication série. 
 * - Elle récupère la donnée reçue (data=UDR0;) dans la variable data et active le drapeau (flag=1;). 
 ...
 */
ISR(USART_RX_vect) {
  // Fonction à exécuter lors de l'interruption de réception USART
  data=UDR0;
  flag=1;
...
/*! \brief 
 * - Le programme principal initialise les interruptions globales et la communication série USART avec un certain taux de bauds. 
 * - Ensuite, dans la boucle infinie, il vérifie périodiquement si des données ont été reçues (indiqué par le drapeau flag). 
